...
#define CHECK_SQLITE_KEY                                                                                                                                    \
    do                                                                                                                                                      \
    {                                                                                                                                                       \
        int result = sqlite3_key(d->access, password.toUtf8().constData(), password.size());                                                                             \
        qDebug() << "result 1 = " << result;                  \
        result = sqlite3_exec(d->access, QStringLiteral("SELECT count(*) FROM sqlite_master LIMIT 1").toUtf8().constData(), nullptr, nullptr, nullptr); \
        if (result != SQLITE_OK)                                                                                                                            \
        {                                                                                                                                                   \
            if (d->access)                                                                                                                                  \
            {                                                                                                                                               \
            ...
            setLastError(qMakeError(d->access, tr("Invalid password."), QSqlError::ConnectionError, result));                                               \
            setOpenError(true);                                                                                                                             \
            return false;                                                                                                                                   \
        }                                                                                                                                                   \
    } while (0)
...
static QMetaType::Type qGetColumnType(const QString &tpName)
{
    ...
    if (typeName == QLatin1String("double") || typeName == QLatin1String("float") || typeName == QLatin1String("real") || typeName.startsWith(QLatin1String("numeric")))
...
protected:
    ...
    bool execBatch(bool arrayBind) Q_DECL_OVERRIDE;
...
bool SQLiteResultPrivate::fetchNext(QSqlCachedResult::ValueCache &values, int idx, bool initialFetch)
{
    ...
    case SQLITE_ERROR:
        ...
        q->setLastError(qMakeError(drv_d_func()->access, QCoreApplication::translate("SQLiteResult", "Unable to fetch row"), QSqlError::ConnectionError, res));
        q->setAt(QSql::AfterLastRow);
        return false;
    case SQLITE_MISUSE:
    case SQLITE_BUSY:
...
bool SQLiteResult::prepare(const QString &query)
{
    ...
    if (res != SQLITE_OK)
    {
        setLastError(qMakeError(d->drv_d_func()->access, QCoreApplication::translate("SQLiteResult", "Unable to execute statement"), QSqlError::StatementError, res));
        d->finalize();
        return false;
    }
    else if (pzTail && !QString(reinterpret_cast<const QChar *>(pzTail)).trimmed().isEmpty())
    {
        setLastError(qMakeError(d->drv_d_func()->access, QCoreApplication::translate("SQLiteResult", "Unable to execute multiple statements at a time"), QSqlError::StatementError, SQLITE_MISUSE));
        d->finalize();
        return false;
    }
    return true;
...
bool SQLiteResult::exec()
{
    ...
    if (res != SQLITE_OK)
    {
        setLastError(qMakeError(d->drv_d_func()->access, QCoreApplication::translate("SQLiteResult", "Unable to reset statement"), QSqlError::StatementError, res));
        d->finalize();
        return false;
    }
    ...
    if (paramCountIsValid)
    {
        for (int i = 0; i < paramCount; ++i)
        {
            ...
            if (res != SQLITE_OK)
            {
                setLastError(qMakeError(d->drv_d_func()->access, QCoreApplication::translate("SQLiteResult", "Unable to bind parameters"), QSqlError::StatementError, res));
                d->finalize();
                return false;
            }
        }
